...
export const useFinCalSupabase = () => {
  ...
  const initializeORTI = useCallback(async () => {
    try {
      ...
      // ✅ Use unified API service instead of direct fetch
      console.log('📊 Getting company data from API Service...');
      const backendData = await apiService.getCompanySummary('ORTI', state.currentYear, true);
      console.log('🏢 API Service response:', backendData);
