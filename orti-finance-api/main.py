...
# ============================================================================
# 🗑️ RESET/CLEAR DATABASE ENDPOINTS
# ============================================================================

@app.delete("/api/companies/{company_name}/reset-entries")
async def reset_company_entries(company_name: str, confirm: bool = False):
    """
    🗑️ RESET/CLEAR all entries for a company
    
    ⚠️ ATTENZIONE: Questo cancellerà TUTTI i dati entries!
    ...
    try:
        if not confirm:
            return {
                "success": False,
                "message": "❌ Operazione non confermata. Usa ?confirm=true per cancellare tutti i dati",
                "warning": "⚠️ Questo comando cancellerà TUTTE le entries della company",
                "example": f"DELETE /api/companies/{company_name}/reset-entries?confirm=true"
            }
        ...
        entries_result = supabase_service.client.table("entries").select("id", count="exact")\
            .eq("subcategories.categories.company_id", finance_manager.company_id).execute()
        
        initial_count = entries_result.count if entries_result.count else 0
        
        # Delete all entries for this company
        delete_result = supabase_service.client.table("entries")\
            .delete()\
            .in_("subcategory_id", [
                sc['id'] for sc in supabase_service.client.table("subcategories")
                .select("id, categories!inner(company_id)")
                .eq("categories.company_id", finance_manager.company_id)
                .execute().data
            ])\
            .execute()
        ...
        return {
            "success": True,
            "message": f"🗑️ Database azzerato per {company_name}",
            "deleted_entries": initial_count,
            "company_id": finance_manager.company_id,
            "timestamp": "2025-01-28",
            "next_step": f"Usa POST /api/companies/{company_name}/bulk-import per importare nuovi dati"
        }
...
@app.post("/api/companies/{company_name}/reset-and-import")
async def reset_and_import(company_name: str, import_data: BulkImportData, confirm: bool = False):
    """
    🔄 RESET database e IMPORT nuovi dati in una volta sola
    
    ⚠️ ATTENZIONE: Cancellerà tutti i dati esistenti e importerà i nuovi
    """
    try:
        if not confirm:
            return {
                "success": False,
                "message": "❌ Operazione non confermata. Usa ?confirm=true per reset + import",
                "warning": "⚠️ Questo comando: 1) Cancella TUTTI i dati, 2) Importa i nuovi dati JSON",
                "example": f"POST /api/companies/{company_name}/reset-and-import?confirm=true"
            }
        
        # Step 1: Reset all entries
        reset_result = await reset_company_entries(company_name, confirm=True)
        
        # Step 2: Import new data
        import_result = await bulk_import_financial_data(company_name, import_data)
        
        return {
            "success": True,
            "message": f"🔄 Reset + Import completato per {company_name}",
            "reset": reset_result,
            "import": import_result,
            "summary": {
                "deleted_entries": reset_result.get("deleted_entries", 0),
                "imported_entries": import_result.get("total_imported", 0),
                "net_change": import_result.get("total_imported", 0) - reset_result.get("deleted_entries", 0)
            }
        }
        
    except Exception as e:
        print(f"❌ Error in reset and import: {e}")
        raise HTTPException(status_code=500, detail=f"Reset and import error: {str(e)}")
